...
export function ElevationProfile({ coords }: Props) {
  ...
  const draw = () => {
    ...
    const y = (a: number) => PAD_TOP + plotH - ((a - minAlt) / altRange) * plotH;
    ...
    for (let d = 0; d <= totalDist / 1000; d += distStep) {
      const px = x(d * 1000);
      ctx.fillText(d.toFixed(distStep < 1 ? 1 : 0) + ' km', px, H - PAD_BOTTOM + 6);
    }
    ...
    for (let i = 0; i < samples; i++) ctx.lineTo(x(sampleDists[i]), y(sampleAlts[i]));
  ...
  const handleMove = (e: MouseEvent) => {
    ...
    for (let i = 1; i < sampleDists.length; i++) {
      if (Math.abs(sampleDists[i] - dist) < Math.abs(sampleDists[idx] - dist)) idx = i;
    }
